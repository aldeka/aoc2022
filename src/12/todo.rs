...
fn can_step(a: char, b: char) {
    let mut digit_a = a.to_digit(10).unwrap();
    let mut digit_a = b.to_digit(10).unwrap();
    if a == get_char("S") {
        adjusted_a = get_char("a").to_digit(10).unwrap();;
    }
    if b == get_char("E") {
        ...
        *current_shortest = path_length;
        ...
        for step in next_steps {
            match map.get(step.0, step.1) {
                ...
                Some(contents) => {
                    if can_step(get_char(contents), current_value) {
